import { Library } from '../../../src/2_OCP/basic/library';
...
const createDocument = (data: DocumentInterface) =>
  new Document(data.id, data.title, data.author, data.publicationYear, DocumentType.BOOK, data.available);
...
describe('Library Class', () => {
  ...
  describe('Document operations', () => {
    ...
    it('should throw an error if a document with the same title already exists', () => {
      expect(() => library.createDocument(document)).toThrow('A document with this title already exists.');
    });
    ...
    it('should throw an error if no document with the given title is found when updating', () => {
      expect(() => library.updateDocument('Non-existing Title', document)).toThrow(
    ...
    it('should throw an error if no document with the given title is found when deleting', () => {
      expect(() => library.deleteDocument('Non-existing Title')).toThrow('No document with this title was found.');
    });
    ...
    it('should return documents that pass the filter function if one is provided', () => {
      const filteredDocuments = library.readDocument((doc) => doc.author === document.author);
      expect(filteredDocuments).toContainEqual(document);
    });
  });

  describe('Document Type Operations', () => {
    it('should correctly identify the type of the document as Book', () => {
      const book = createDocument(documents[1]);
      library.createDocument(book);
      const retrievedDocuments = library.readDocument((doc) => doc.type === DocumentType.BOOK);
      expect(retrievedDocuments).toContainEqual(book);
      expect(retrievedDocuments[0].type).toBe(DocumentType.BOOK);
    });
  });

  describe('Book class', () => {
    it('should return "Book" as its type using getType()', () => {
      // Étape 1: Créer une instance de Book
      const myBook = new Book(
        documents[0].id,
        documents[0].title,
        documents[0].author,
        documents[0].publicationYear,
        documents[0].type,
        documents[0].available,
      );
      const type = myBook.getType();
      expect(type).toBe(DocumentType.BOOK);
    });
  });
  ...
  describe('Student operations', () => {
    ...
    it('should throw an error if a student with the same ID already exists', () => {
      expect(() => library.createStudent(student)).toThrow('A student with this ID already exists.');
    });
    ...
    it('should throw an error if no student with the given ID is found when updating', () => {
      expect(() => library.updateStudent(3, student)).toThrow('No student with this ID was found.');
    });
    ...
    it('should throw an error if no student with the given ID is found when deleting', () => {
      expect(() => library.deleteStudent(3)).toThrow('No student with this ID was found.');
    });
    ...
    it('should return students that pass the filter function if one is provided', () => {
      const filteredStudents = library.readStudent((stu) => stu.firstName === student.firstName);
